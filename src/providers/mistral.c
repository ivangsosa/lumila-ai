...
LumilaProvider *mistral_provider_new(void)
{
    ...
    provider->base.session = soup_session_new_with_options(
        "timeout", (lumila_config_get_timeout(LUMILA_PROVIDER_MISTRAL) > 0 ? lumila_config_get_timeout(LUMILA_PROVIDER_MISTRAL) : 60),
        NULL);
#else
    provider->base.session = soup_session_new_with_options(
        SOUP_SESSION_TIMEOUT, (lumila_config_get_timeout(LUMILA_PROVIDER_MISTRAL) > 0 ? lumila_config_get_timeout(LUMILA_PROVIDER_MISTRAL) : 60),
        NULL);
#endif
...
#if SOUP_CHECK_VERSION(3, 0, 0)
    SoupMessage *msg = soup_message_new("POST", lumila_config_get_endpoint(LUMILA_PROVIDER_MISTRAL) ? lumila_config_get_endpoint(LUMILA_PROVIDER_MISTRAL) : MISTRAL_API_BASE);
